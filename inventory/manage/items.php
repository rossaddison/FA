...
include(dirname(__DIR__, 2) . "/reporting/includes/tcpdf.php");
...
include_once(dirname(__DIR__, 2) . "/fixed_assets/includes/fixed_assets_db.inc");
...
if (isset($_POST['addupdate'])) 
{
  ...
  if (get_post('fixed_asset')) {
    ...
    $move_row = row_or_empty(get_fixed_asset_move(post_scalar('NewStockID'), ST_SUPPRECEIVE));
  ...
  item_tax_types_list_row(_("Item Tax Type:"), 'tax_type_id', null, !$new_item && item_type_inactive(get_post('tax_type_id')));
	...
	if (get_post('fixed_asset')) {
		...
		if (!$new_item) {
			...
			label_row(_("Depreciations").":", price_format((float)$_POST['purchase_cost'] - (float)$_POST['material_cost']), "", "align='right'");
...
tabbed_content_start('tabs', $tabs);
	...
	switch (get_post('_tabs_sel')) {
		...
		case 'sales_pricing':
			...
			$_GET['page_level'] = 1;
			include_once(dirname(__DIR__, 2) . "/inventory/prices.php");
			break;
		case 'purchase_pricing':
			$_GET['stock_id'] = $stock_id;
			$_GET['page_level'] = 1;
			include_once(dirname(__DIR__, 2) . "/inventory/purchasing_data.php");
			break;
		case 'standard_cost':
			$_GET['stock_id'] = $stock_id;
			$_GET['page_level'] = 1;
			include_once(dirname(__DIR__, 2) . "/inventory/cost_update.php");
			break;
		case 'reorder_level':
			...
			$_GET['page_level'] = 1;
			$_GET['stock_id'] = $stock_id;
			include_once(dirname(__DIR__, 2) . "/inventory/reorder_level.php");
			break;
		case 'movement':
			if (!is_inventory_item($stock_id))
				break;
			$_GET['stock_id'] = $stock_id;
			include_once(dirname(__DIR__, 2) . "/inventory/inquiry/stock_movements.php");
			break;
		case 'status':
			$_GET['stock_id'] = $stock_id;
			include_once(dirname(__DIR__, 2) . "/inventory/inquiry/stock_status.php");
			break;
		case 'attachments':
