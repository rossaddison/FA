 ...
 * @return null|string
 */
function getTransactions(string|int|array|null $supplier_id, string|array|null $date)
{
	$date = date2sql($date);
...
function print_supplier_details_listing(): void
{
	...
	while ($myrow=db_fetch($result))
	{
		...
		if ($printsupplier)
		{
			...
			{
				$dim = row_or_empty(get_dimension($myrow['dimension_id']));
			...
			{
				$dim = row_or_empty(get_dimension($myrow['dimension2_id']));
