...
if (isset($_POST['go']) || isset($_POST['show']))
{
	...
	if ($input_error == 0)
	{
		...
		$lastdate = ($freq == 1 ? add_days($date_, 7.0*$per) :
			($freq == 2 ? add_days($date_, 14.0*$per) :
			($freq == 3 ? end_month(add_months($date_, $per)) : 
			end_month(add_months($date_, 3.0*$per)))));
		...
		if ($input_error == 0)
		{
			...
			if ($am * (float)$periods != $amount)
				$am0 = $am + (float)$amount - $am * (float)$periods;
			else
				$am0 = $am;
			if (get_post('memo_') != "")
				$memo = $_POST['memo_'];
			...
			for ($i = 0; $i < $periods; $i++)
			{
				...
				else
				{
					...
					label_cell($date);
					label_cell((string)$_POST['acc_act'] . " " . get_gl_account_name(post_scalar('acc_act')));
					if ($dim > 0)
						label_cell("");
					if ($dim > 1)
						label_cell("");
					display_debit_or_credit_cells($am0 * (float)(-1));
					label_cell($memo);
					alt_table_row_color($k);
					label_cell($date);
					label_cell((string)$_POST['res_act'] . " " . get_gl_account_name(post_scalar('res_act')));
					if ($dim > 0)
